...
def drawMatchPoints(img1, pts1, img2, pts2, desc1, desc2, cvBFSpp, win, display_scale = 1):
  ...
  print("[superPoint]==>pts1.sz:{0}, desc1.sz:{1}, pts2.sz:{2}, desc2.sz:{3}, matches.sz:{4}".format(pts1.shape[1], desc1.T.shape, pts2.shape[1], desc2.T.shape, len(matches)))
  ...
  for match in matches:
    ...
    cv2.circle(stereo_img, ptL, 1, (0, 255, 0), -1, lineType=16)
    cv2.circle(stereo_img, ptR, 1, (0, 255, 0), -1, lineType=16)
    cv2.line(stereo_img, ptL, ptR, (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)), thickness=1, lineType=cv2.LINE_AA)
...
def detectAndCompute(img1, img2, orb, cvBFORB, display_scale = 1):
    ...
    print("[ORB       ]==>pts1.sz:{0}, desc1.sz:{1}, pts2.sz:{2}, desc2.sz:{3}, matches.sz:{4}".format(len(kpts1), desc1.shape, len(kpts2), desc2.shape, len(matches)))
    rows, cols = img1.shape
    cols = int(cols*2*display_scale)
    rows = int(rows*display_scale)
...
class SuperPointNet(torch.nn.Module):
  ...
  def forward(self, x):
    ...
    # new_desc = desc.permute(0,3,2,1)
    # print("new_desc.shape: ", new_desc.shape)
    # # print(desc)
    # assert 0
...
class VideoStreamer(object):
  ...
  """
  def __init__(self, basedir, camid, height, width, skip, img_glob, list_path):
    self.cap = []
    self.camera = False
    ...
    if basedir == "camera/" or basedir == "camera":
      ...
      if (type(self.cap) == list or not self.cap.isOpened()) and (lastbit == '.mp4'):
        ...
        self.listing.sort()
        self.listing = self.listing[::self.skip]
        self.maxlen = len(self.list_path)
        if self.maxlen == 0:
  ...
  def next_frame(self):
    ...
    if self.camera:
      ...
      # image_file = "/WORK/CODE/DL/relocation/superpoint/onnx/27_1614047187687155.jpg"
      # image_file = "/home/indemind/Code/CLionprojects/Parker/superpoint/input_img_r.jpg"
      image_file = self.list_path[self.i]
...
if __name__ == '__main__':
  ...
  list_path_left, list_path_right, list_path_depth = get_list_path()
  # This class helps load input images from different sources.
  vs = VideoStreamer(opt.input1, opt.camid, opt.H, opt.W, opt.skip, opt.img_glob, list_path_left)
  vs2 = VideoStreamer(opt.input2, opt.camid, opt.H, opt.W, opt.skip, opt.img_glob, list_path_right)
  ...
  while vs.i <= vs.maxlen:
    ...
    pts_disparity = compute_disparity(img, pts, img2, pts2, desc, desc2, cvBFSpp)

    # if there are no feature points, process the next image
    if pts_disparity is None:
      continue
    else:
      # compute the depth
      pts_depth = compute_depth_with_disparity(pts_disparity)
      # mark_depth(pts_depth)

      # revise the depth
      revise_depth(pts_depth, list_path_depth[vs.i - 1])

    if vs.i % 1000 == 0:
      print("%d images are done!!!" % vs.i)

    # print("revise depth of image: " + list_path_depth[vs.i - 1].split("/")[-1])

    # Display visualization image to screen.
    """if not opt.no_display:
      show = np.hstack([out1, out2])
      cv2.imshow(win1, show)
      drawMatchPoints(img, pts, img2, pts2, desc, desc2, cvBFSpp, win3, opt.display_scale)
      detectAndCompute(img, img2, orb, cvBFORB, opt.display_scale);
